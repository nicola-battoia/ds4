...
.PHONY: all help clean test test-memory-plan test-metal-session-batch test-mxfp4-cuda test-cuda-session-batch test-cuda-mixed-batch dspark-acceptance dspark-verify-depth mtp-verify-depth metal-memory-probe run-metal-memory-probe cpu cuda cuda-spark cuda-generic cuda-regression strix-halo rocm
...
help:
	...
	@echo "  make test-memory-plan  Run header-only GGUF memory-planner tests"
...
test: ds4_test ds4_agent_test ds4-eval q4k-dot-test mxfp4-dot-test test-memory-plan \
